...
| `REPO_URLS` | Comma-separated list of repo URLs to run **multiple runners in a single container**, one per repo (multi-repo mode). Forces `RUNNER_SCOPE=repo`. Example: `REPO_URLS=https://github.com/me/repo1,https://github.com/me/repo2`. Auth via `ACCESS_TOKEN` (PAT, recommended) or `APP_ID`+`APP_PRIVATE_KEY`+`APP_LOGIN` — the access token is minted/refreshed once and reused across repos. Each runner gets its own `/actions-runner-N` tree (hardlinked from `/actions-runner` to save disk), its own runner name (`<RUNNER_NAME_PREFIX>-<repo>-<suffix>`), and its own workdir under `RUNNER_WORKDIR`. Incompatible with `CONFIGURED_ACTIONS_RUNNER_FILES_DIR` (reusage). Single-repo mode (using `REPO_URL`) is unchanged when `REPO_URLS` is empty. |
